  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight md:text-3xl">Welcome back, {user?.firstName}.</h1>
          ...
          <>
                ...
                .map(([currency, total]) => (
                  <Card key={currency} className="card-lift bg-brand-gradient text-white">
                    <CardHeader className="pb-2">
                      <CardDescription className="text-white/70">Total balance ({currency})</CardDescription>
                      <CardTitle className="text-3xl text-white">{formatMoney(total, currency)}</CardTitle>
                    </CardHeader>
                  </Card>
                ))
            ) : (
            ...
            <Card className="card-lift">
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">{(notifications?.items ?? []).filter((n) => !n.readAt).length}</CardTitle>
              </CardHeader>
            </Card>
          </>
        )}
              ...
              {isLoading ? (
                ...
                accounts.map((account) => (
                  ...
                  >
                    ...
                    <p className="text-lg font-semibold">{formatMoney(account.availableBalance, account.currencyCode)}</p>
                  </Link>
                ))
              ) : (
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>{firstAccount ? `From ${firstAccount.accountNumber}` : 'No account selected'}</CardDescription>
              </div>
              {firstAccount && (
            ...
            <CardContent className="space-y-1">
              {recentTransactions.length === 0 ? (
                ...
                recentTransactions.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between rounded-lg px-2 py-3 hover:bg-accent">
                    <div className="flex items-center gap-3">
                      <span
                        className={
                      ...
                      <div>
                        <p className="text-sm font-medium capitalize">{tx.transactionType.toLowerCase()}</p>
                        <p className="text-xs text-muted-foreground">{formatDateTime(tx.createdAt)}</p>
                      </div>
                    </div>
                    <p className={tx.transactionType === 'DEPOSIT' ? 'font-semibold text-success' : 'font-semibold text-destructive'}>
                      {tx.transactionType === 'DEPOSIT' ? '+' : '-'}
              ...
              {(notifications?.items ?? []).slice(0, 3).length === 0 ? (
                ...
                (notifications?.items ?? []).slice(0, 3).map((n) => (
                  <div key={n.id} className="rounded-lg px-2 py-2 hover:bg-accent">
                    <p className="truncate text-sm font-medium text-foreground">{n.subject ?? 'Notification'}</p>
      ...
      <p className="text-xs text-muted-foreground">Last updated {formatDateTime(new Date().toISOString())}</p>
    </div>
  );
}
