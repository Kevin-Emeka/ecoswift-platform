...
/**
 * A pure background worker plus a health endpoint — no user-facing HTTP
 * API, so no `AuthzModule`/`AuthClientModule` (nothing to authenticate;
 * `EmailWorker` is a BullMQ consumer, not a controller).
 */
@Module({
  imports: [
